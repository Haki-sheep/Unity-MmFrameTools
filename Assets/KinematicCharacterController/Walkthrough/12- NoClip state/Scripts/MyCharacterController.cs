...
namespace KinematicCharacterController.Walkthrough.NoClipState
{
    /// <summary>
    /// 角色状态枚举
    /// 新增NoClip（无碰撞/观察者）状态，用于实现自由飞行穿墙模式
    /// </summary>
    ...
    /// <summary>
    /// 玩家角色输入结构体
    /// 新增JumpHeld、CrouchHeld、NoClipDown输入，用于NoClip模式的垂直移动和状态切换
    /// </summary>
    public struct PlayerCharacterInputs
    {
        ...
        public bool JumpHeld;          // 跳跃按键按住（持续输入，用于NoClip上升）
        public bool CrouchDown;        // 蹲伏按键按下（帧）
        public bool CrouchUp;          // 蹲伏按键抬起（帧）
        public bool CrouchHeld;        // 蹲伏按键按住（持续输入，用于NoClip下降）
        public bool NoClipDown;        // NoClip按键按下（帧，用于切换状态）
    }

    /// <summary>
    /// 自定义角色控制器核心类（含NoClip状态）
    /// 实现ICharacterController接口，支持默认移动和无碰撞飞行两种状态
    /// </summary>
    public class MyCharacterController : MonoBehaviour, ICharacterController
    {
        ...
        [Header("NoClip设置")]
        public float NoClipMoveSpeed = 10f; // NoClip模式下的移动速度
        public float NoClipSharpness = 15; // NoClip模式下的移动响应锐度
        ...
        private bool _jumpInputIsHeld = false; // 跳跃键是否按住（用于NoClip上升）
        private bool _crouchInputIsHeld = false; // 蹲伏键是否按住（用于NoClip下降）
        ...
        public void OnStateExit(CharacterState state, CharacterState toState)
        {
            switch (state)
            {
                ...
                case CharacterState.NoClip:
                    {
                        // 重新开启胶囊碰撞体激活、移动碰撞解决和地面检测，恢复正常物理
        ...
        {
            // 处理输入触发的状态切换（按下NoClip键时在Default和NoClip之间切换）
            ...
            switch (CurrentCharacterState)
            {
                ...
                case CharacterState.NoClip:
                    {
                        // NoClip模式下，移动输入直接基于相机方向（实现自由视角飞行）
                ...
                case CharacterState.NoClip:
                    {
                        ...
                        // 计算目标速度：水平移动输入 + 垂直输入，归一化后乘以NoClip速度
