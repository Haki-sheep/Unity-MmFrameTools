...
namespace KinematicCharacterController.Walkthrough.WallJumping
{
    ...
    // 自定义角色控制器（墙跳版）：实现ICharacterController接口，处理角色移动、旋转、跳跃（含二段跳/墙跳）核心逻辑
    public class MyCharacterController : MonoBehaviour, ICharacterController
    {
        ...
        public float JumpPreGroundingGraceTime = 0f; // 跳跃预接地容错时间（提前按跳保留请求）
        public float JumpPostGroundingGraceTime = 0f; // 跳跃后接地容错时间（土狼时间，离地短时间仍可跳）
        ...
        private bool _jumpConsumed = false; // 基础跳跃请求是否已被消耗（防止连续跳）
        ...
        private bool _doubleJumpConsumed = false; // 二段跳是否已消耗
        private bool _canWallJump = false; // 当前是否满足墙跳条件
        private Vector3 _wallJumpNormal; // 墙跳时的墙面法线（决定跳跃方向）
        ...
        /// 这是唯一允许设置角色速度的地方，包含地面/空中移动、跳跃（基础/二段/墙跳）逻辑
        /// </summary>
        ...
        public void UpdateVelocity(ref Vector3 currentVelocity, float deltaTime)
        {
            ...
            // ========== 跳跃逻辑（含二段跳/墙跳） ==========
            {
                ...
                if (_jumpRequested)
                {
                    ...
                    if (AllowDoubleJump)
                    {
                        // 条件：基础跳已消耗 + 二段跳未消耗 + 不在地面（或斜坡滑动时不允许跳）
                    ...
                    {
                        // 计算跳跃方向：墙跳用墙面法线 → 斜坡滑动用地面法线 → 默认角色向上
        ...
        /// 核心：检测墙跳条件
        /// hitCollider碰撞到的物体碰撞体
        /// hitNormal碰撞点法线,hitPoint碰撞点位置
        /// hitStabilityReport碰撞稳定性报告
        /// </summary>
        public void OnMovementHit(Collider hitCollider, Vector3 hitNormal, Vector3 hitPoint, ref HitStabilityReport hitStabilityReport)
        {
            // 墙跳条件：开启墙跳 + 不在稳定地面 + 碰撞到非稳定的障碍物（墙面）
        ...
        /// <summary>
        /// 给角色添加额外速度（由KinematicCharacterMotor调用）
        /// 空方法，暂无需处理
        /// </summary>
