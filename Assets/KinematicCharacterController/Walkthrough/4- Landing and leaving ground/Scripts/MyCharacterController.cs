...
namespace KinematicCharacterController.Walkthrough.LandingLeavingGround
{
    ...
    // 自定义角色控制器（落地/离开地面检测版）：实现ICharacterController接口
    // 核心功能：地面/空中移动、角色朝向控制、跳跃（含二段跳/墙跳）、落地/离开地面事件回调
    public class MyCharacterController : MonoBehaviour, ICharacterController
    {
        ...
        public float StableMovementSharpness = 15; // 地面移动响应锐度（值越大加速/减速越快，过渡越生硬）
        ...
        public void SetInputs(ref PlayerCharacterInputs inputs)
        {
            ...
            // 将移动输入转换为「基于相机朝向」的世界空间向量（玩家按W始终向相机前方移动）
            _moveInputVector = cameraPlanarRotation * moveInputVector;
            // 记录角色需要朝向的目标方向（用于UpdateRotation平滑转向）
        ...
        /// 核心逻辑：区分地面/空中移动规则，处理跳跃（基础/二段/墙跳）、重力、阻力
        /// </summary>
        ...
        /// 核心作用：检测「落地」和「离开地面」事件，触发对应回调
        /// </summary>
        /// <param name="deltaTime">帧时间（秒）</param>
        public void PostGroundingUpdate(float deltaTime)
        {
            // 检测落地事件：当前稳定接地 且 上一帧未稳定接地
            if (Motor.GroundingStatus.IsStableOnGround && !Motor.LastGroundingStatus.IsStableOnGround)
            {
                OnLanded();
            }
            // 检测离开地面事件：当前未稳定接地 且 上一帧稳定接地
        ...
        /// <summary>
        /// 离开稳定地面事件回调（可重写扩展：添加跳跃音效、空中状态初始化等）
        /// </summary>
