...
namespace KinematicCharacterController.Walkthrough.ChargingState
{
    ...
    public struct PlayerCharacterInputs
    {
        ...
        public bool ChargingDown; // 冲锋按键按下（帧）
    }

    /// <summary>
    /// 自定义角色控制器核心类（含冲锋状态）
    /// 实现ICharacterController接口，支持默认移动和冲锋两种状态
    /// </summary>
    public class MyCharacterController : MonoBehaviour, ICharacterController
    {
        ...
        [Header("冲锋设置")]
        public float ChargeSpeed = 15f; // 冲锋速度
        public float MaxChargeTime = 1.5f; // 最大冲锋持续时间（超时自动停止）
        public float StoppedTime = 1f; // 冲锋停止后的停顿时间（之后切回默认状态）
        ...
        private Vector3 _currentChargeVelocity; // 当前冲锋速度向量（缓存自角色前方方向）
        private bool _isStopped; // 是否处于冲锋停止状态
        private bool _mustStopVelocity = false; // 是否需要强制清零速度
        private float _timeSinceStartedCharge = 0; // 冲锋开始后经过的时间
        private float _timeSinceStopped = 0; // 冲锋停止后经过的时间
        ...
        {
            // 处理输入触发的状态切换（按下冲锋键时切换到Charging状态）
